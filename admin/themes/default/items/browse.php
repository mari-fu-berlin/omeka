...
$reqItemTypeId = trim($request->getParam('type'));

$addItemStr =  __('Add an Item');
$searchItemStr =  __('Search Items');
$addItemHrefStr = html_escape(url('items/add'));

if (isset($reqItemTypeId) && !empty($reqItemTypeId)) {
    $itemType = get_db()->getTable('ItemType')->find($reqItemTypeId);
    $addItemStr =  $itemType->name . ' ' . __('hinzufügen');
    // $searchItemStr =  __('Search Items');
    $addItemHrefStr = html_escape(url('items/add?type=' . $itemType['id']));
    // var_dump($itemType);
}

$addNewLinkStr = '<a href="' . $addItemHrefStr . '" class="add button small green">' . $addItemStr . '</a>';
